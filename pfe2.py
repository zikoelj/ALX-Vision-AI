...
def convert_video(input_path, output_path):
    try:
        ...
        raise RuntimeError(f"Command ffmpeg failed: {e}")
        ...
        if request.method == 'POST':
            ...
            if file and allowed_file(file.filename):
                ...
                if filename.rsplit('.', 1)[1].lower() in {'mp4', 'avi', 'mov'}:
                    ...
                    if filename.rsplit('.', 1)[1].lower() != 'mp4':  # If not already MP4
                        output_video_path = convert_video(filename, output_video_path)
